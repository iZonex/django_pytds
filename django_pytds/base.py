...
from . import pytdsdbapi as Database        # PyTDS stuff
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def get_new_connection(self, conn_params):
        ...
        settings_dict = self.settings_dict.copy()
        db_name = settings_dict['NAME'].strip()
        db_host = settings_dict['HOST'] or '127.0.0.1'
        db_port = settings_dict['PORT']
        db_user = settings_dict['USER']
        db_password = settings_dict['PASSWORD']
        autocommit = settings_dict.get('OPTIONS', {}).get('autocommit', False)
        use_mars = settings_dict.get('OPTIONS', {}).get('use_mars', False)
